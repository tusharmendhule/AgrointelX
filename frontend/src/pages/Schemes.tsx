        ...
        <div className="flex flex-wrap items-center gap-2">
          <div className="relative text-xs">
            ...
            <input 
              ...
              className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl py-2 pl-9 pr-3 text-slate-900 dark:text-slate-100 focus:outline-none focus:border-emerald-500"
            />
          </div>
          ...
          <select
            ...
            className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl py-2 px-3 text-xs text-slate-900 dark:text-slate-100 focus:outline-none focus:border-emerald-500"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {filteredSchemes.length === 0 ? (
          ...
          filteredSchemes.map((s) => (
            <div 
              key={s.id}
              className="p-6 rounded-3xl bg-white dark:bg-slate-900/40 border border-slate-200 dark:border-slate-800/80 dark:backdrop-blur-md flex flex-col justify-between group hover:border-emerald-500/20 transition-all shadow-sm"
            >
              <div>
                ...
                <h3 className="text-sm font-bold text-slate-900 dark:text-slate-100 group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition-colors">{s.name}</h3>
                <p className="text-xs text-slate-600 dark:text-slate-400 mt-2 leading-relaxed">{s.description}</p>
                
                {/* Eligibility Checkbox */}
                <div className="p-3 bg-slate-50 dark:bg-slate-950/40 border border-slate-200 dark:border-slate-800/80 rounded-xl mt-4 text-[11px]">
                  <span className="text-[10px] uppercase font-mono font-bold text-slate-500 dark:text-slate-400 block mb-1">{t("schemes.eligibilityScan", "Eligibility Scan")}</span>
                  <p className="text-slate-700 dark:text-slate-300 font-semibold">{checkEligibility(s)}</p>
                </div>
              </div>
              ...
              <div className="mt-6 border-t border-slate-200 dark:border-slate-800/80 pt-4 flex items-center justify-between">
                <div className="flex flex-col text-[10px] text-slate-500 dark:text-slate-400">
                  <span className="font-semibold text-slate-500 dark:text-slate-400">{t("schemes.officialPortal", "Official Portal")}:</span>
                  <span className="text-[9px] font-mono text-emerald-600 dark:text-emerald-500/80 hover:underline">
                    {getDomain(s.link)}
                  </span>
                </div>
                ...
                {s.applied ? (
                  <button 
                    disabled 
                    className="px-4 py-2 bg-slate-100 dark:bg-slate-800 text-emerald-600 dark:text-emerald-500 font-bold rounded-xl text-[11px] flex items-center gap-1"
                  >

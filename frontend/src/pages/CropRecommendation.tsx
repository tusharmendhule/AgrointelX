        ...
        <div className="lg:col-span-2 p-6 rounded-3xl bg-slate-900/40 border border-slate-200 dark:border-slate-800/80 backdrop-blur-md">
          ...
          <form onSubmit={handleRecommend} className="space-y-4 text-xs">
            ...
            <div className="grid grid-cols-3 gap-3">
              <div>
                <label className="block text-slate-400 font-semibold mb-1">{t("crop.nitrogen", "Nitrogen (N)")}</label>
                <input 
                  type="number" 
                  value={nitrogen} 
                  onChange={(e) => setNitrogen(e.target.value)}
                  min="0" max="200" required
                  className="w-full bg-slate-950/60 border border-slate-800 rounded-xl py-2.5 px-3 font-mono font-bold text-center text-slate-100 focus:outline-none focus:border-emerald-500"
                />
              </div>

              <div>
                <label className="block text-slate-400 font-semibold mb-1">{t("crop.phosphorus", "Phosphorus (P)")}</label>
                <input 
                  type="number" 
                  value={phosphorus} 
              ...
              <div>
                <label className="block text-slate-400 font-semibold mb-1">{t("crop.potassium", "Potassium (K)")}</label>
                <input 
                  type="number" 
                  value={potassium} 
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-slate-400 font-semibold mb-1">{t("crop.soilPh", "Soil pH Factor")}</label>
                <input 
                  type="number" 
                  step="0.1" 
                  value={ph} 
              ...
              <div>
                <label className="block text-slate-400 font-semibold mb-1">{t("crop.soilTaxonomy", "Soil Taxonomy")}</label>
                <select 
                  value={soilType} 
            ...
            {weatherBrief && (
              <div className="p-3 bg-slate-950/60 border border-slate-800/80 rounded-xl flex items-center justify-between text-[11px] text-slate-400">
          ...
          <AnimatePresence mode="wait">
            {!result ? (
              ...
              >
                ...
                <h4 className="font-bold text-slate-300">{t("crop.reportPendingTitle", "Soil Diagnostic Report Pending")}</h4>
              ...
              >
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pb-4 border-b border-slate-200 dark:border-slate-800/80">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <span className="text-[10px] uppercase font-mono font-bold text-emerald-400">{t("crop.modelRecommendation", "Model Recommendation")}</span>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-xs">
                  <div className="p-4 rounded-2xl bg-white dark:bg-slate-950/40 border border-slate-200 dark:border-slate-800">
                    <span className="text-slate-400 font-medium">{t("crop.cycleDuration", "Cycle Duration")}</span>
                    <p className="font-bold text-base mt-1 text-slate-100 font-mono">{result.growthDurationDays} {t("common.days", "Days")}</p>
                  ...
                  <div className="p-4 rounded-2xl bg-white dark:bg-slate-950/40 border border-slate-200 dark:border-slate-800">
                    ...
                    <p className={`font-bold text-base mt-1 font-mono ${result.marketDemand === "High" ? "text-emerald-400" : "text-amber-400"}`}>{t(`marketDemand.${result.marketDemand}`, result.marketDemand)}</p>
                  ...
                  <div className="p-4 rounded-2xl bg-white dark:bg-slate-950/40 border border-slate-200 dark:border-slate-800">
                    ...
                    <p className="font-bold text-xs mt-1.5 text-slate-100 truncate">{result.alternativeCrops.join(", ")}</p>
                ...
                <div className="space-y-4 text-xs leading-relaxed">
                  <div className="p-4 rounded-2xl bg-slate-950/40 border border-slate-800/80">
                    <h4 className="font-bold text-slate-200 mb-1 flex items-center gap-1.5">
                      <ShieldCheck className="h-4 w-4 text-emerald-400" /> {t("crop.suitabilityDiagnosis", "Suitability Diagnosis (Explainable AI)")}
                    </h4>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div className="p-4 rounded-2xl bg-slate-950/40 border border-slate-800/80">
                      <h4 className="font-bold text-slate-200 mb-1 flex items-center gap-1.5">
                        <Award className="h-4 w-4 text-emerald-400" /> {t("crop.fertilizerTips", "Fertilizer Adjustment Tips")}
                      </h4>
                    ...
                    <div className="p-4 rounded-2xl bg-slate-950/40 border border-slate-800/80">
                      <h4 className="font-bold text-slate-200 mb-1 flex items-center gap-1.5">
                        <TrendingUp className="h-4 w-4 text-emerald-400" /> {t("crop.irrigationScheduling", "Water & Irrigation Scheduling")}
                      </h4>

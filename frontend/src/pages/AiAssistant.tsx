...
export default function AiAssistant() {
  ...
  const handleSend = async (textToSend?: string) => {
    ...
    try {
      ...
      const res = await api.chat(chatLogs);
  ...
  return (
    <div className="space-y-6 font-sans text-slate-100 max-w-5xl mx-auto h-[calc(100vh-8rem)] flex flex-col">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 shrink-0">
        ...
        <button
          onClick={clearChat}
          className="flex items-center gap-1.5 px-3.5 py-2 bg-slate-900 border border-slate-800 hover:border-rose-500/40 text-xs text-slate-400 hover:text-rose-400 rounded-xl transition-all font-semibold"
      ...
      <div className="flex-1 min-h-0 grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="lg:col-span-1 flex flex-col gap-4 overflow-y-auto hidden lg:flex">
          <div className="p-4 bg-slate-900/40 border border-slate-800 rounded-3xl backdrop-blur-md space-y-4">
          ...
          <div className="p-4 bg-slate-900/20 border border-slate-900 rounded-3xl space-y-2 text-[10px] text-slate-400">
        ...
        <div className="lg:col-span-3 flex flex-col rounded-3xl bg-slate-900/40 border border-slate-200 dark:border-slate-800/80 backdrop-blur-md overflow-hidden h-full">
          ...
          <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-6">
              ...
              return (
                ...
                >
                  ...
                  <div className={`max-w-[85%] rounded-2xl px-4 py-3 md:px-5 md:py-4 shadow-sm relative ${
                    isBot 
                      ? "bg-slate-950/40 border border-slate-800 rounded-tl-none" 
                  ...
                  {/* User Avatar */}
                  {!isBot && (
                    <div className="h-8 w-8 rounded-xl bg-slate-800 border border-slate-700 flex items-center justify-center text-slate-300 shrink-0 shadow-md font-extrabold text-xs">
                      U
                    </div>
                  )}
                </div>
            ...
            {loading && (
              <div className="flex gap-3 justify-start">
                ...
                <div className="bg-slate-950/40 border border-slate-800 rounded-2xl rounded-tl-none px-4 py-3 text-xs text-slate-500 dark:text-slate-400 flex items-center gap-1.5 shadow-sm">
          ...
          <div className="lg:hidden px-4 pt-2 flex gap-2 overflow-x-auto whitespace-nowrap scrollbar-none shrink-0 pb-2">
            {suggestions.map((s, idx) => (
              <button
                ...
                onClick={() => handleSuggestClick(s.prompt)}
                disabled={loading}
                className="inline-flex items-center gap-1 px-3 py-1.5 bg-slate-950/55 border border-slate-800 rounded-full text-[10px] text-slate-300 font-medium active:scale-95 transition-all"
              >
                {s.label}
              </button>
            ))}
          ...
          {/* Input Panel */}
          <div className="p-4 border-t border-slate-200 dark:border-slate-800/80 bg-slate-950/40 shrink-0">
            <div className="flex items-center gap-2">
              <button
                ...
                className={`p-3.5 rounded-2xl transition-all border shrink-0 ${
                  isRecording 
                    ...
                    : "bg-slate-950 border-slate-800 text-slate-400 hover:text-slate-100 hover:border-slate-700 hover:bg-slate-900"
                }`}
              ...
              <input 
                ...
                className="flex-1 bg-slate-950 border border-slate-800 rounded-2xl py-3.5 px-4 text-xs md:text-sm text-slate-100 placeholder-slate-500 focus:outline-none focus:border-emerald-500 focus:bg-slate-950 transition-colors"
              />
              ...
              <button
                ...
                className="p-3.5 bg-emerald-600 hover:bg-emerald-500 disabled:bg-slate-900 disabled:text-slate-600 disabled:shadow-none text-white rounded-2xl transition-all active:scale-95 shadow-md shadow-emerald-600/10 shrink-0"
              >
